...
clusterExport(cl, c("lambda_poisson", "alpha_pareto", "beta_pareto", "deducible", "M", "simular_S", "rpois", "rpareto", "sapply", "unlist", "limAseg"))
...
$$E[VW]/E[V] = \frac{`r mean_promedio_S`}{`r mean(mydata$Intermedio)`}$$

`r mean_promedio_S/mean(mydata$Intermedio)`.
...
Por lo tanto, la cuota de tarifa que proponemos para el seguro de instrumentos musicales es de `r mean_promedio_S/(mean(mydata$Intermedio) * (1-.5))` por cada instrumento asegurado.
...
$$RRC\% = \frac{\text{primaRiesgo}}{\text{primaTarifa}} = `r (1-.4)`$$
...
Como no tenemos experiencia en el mercado, proponemos un ROPC del 100% de la prima de riesgo promedio, es decir, `r (1-.4) ` por cada póliza vendida. Ya cuando generemos experiencia, podremos ajustar este parámetro.
