...
```{r}
deducible <- 2000
simulacionUnif <- runif(1000000, 0, 1)

verifProb <- simulacionUnif >= ppareto(deducible, params_pareto$par[1], params_pareto$par[2])

simulacionesPerdida <- ifelse(verifProb,qpareto(simulacionUnif, params_pareto$par[1], params_pareto$par[2]) -2000, 0)

mean(simulacionesPerdida)
```

```{r}
(cuota <- .8*mean(simulacionesPerdida)*lambda_poisson/mean(mydata$Intermedio))
```
...
```{r}
deducible <- 1000
simulacionUnif <- runif(100000, 0, 1)

verifProb <- simulacionUnif >= plnorm(deducible, params_lognormal$par[1], params_lognormal$par[2])

simulacionesPerdida <- ifelse(verifProb,qlnorm(simulacionUnif, params_lognormal$par[1], params_lognormal$par[2])-1000 , 0)
